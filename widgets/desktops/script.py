...
class Icon:
    ...
    def icon_from_path(
        ...
        for ext in exts:
            icon_path = Path.home() / f".cache/eww/{window_class}-{window_class2}{ext}"
            if icon_path.exists():
        ...
        for path in paths:
            for f in path.rglob("*"):
                if f.is_file() and f.suffix in exts and (
                    any([f.stem.lower() == s.lower() for s in target_stems])
                    # any([f.stem.endswith(s) for s in target_stems]) or
                    # any([f.stem.startswith(s) for s in target_stems])
                    ):
                    p = (
                    ...
                    p.symlink_to(f)
                    Icon._ICON_PATH_CACHE[(window_class, window_class2)] = str(p)

                    return str(p)
        return ""

    @staticmethod
    def icon_from_xprop(window_id, window_class, window_class2):
        icon_path = Path.home() / ".cache" / "eww" / f"{window_class}-{window_class2}.png"
        ...
        if (
            window_class2 == "Google-chrome"
            and window_class.startswith("crx_")
            and window_class.startswith("jetbrains")
        ):
    ...
    @staticmethod
    def get_path2(w_id, w_class, w_class2):
        if w_class in Icon._ICON_PATH_CACHE:
            return Icon._ICON_PATH_CACHE[(w_class, w_class2)]

        if w_class2 == "Google-chrome" and w_class.startswith("crx_"):
            icon_path = Icon.icon_from_path(w_class, w_class2)
            if icon_path:
                return icon_path
        else:
            icon_path = Icon.icon_from_xprop(w_id, w_class, w_class2)
            if icon_path:
                return icon_path
            return Icon.icon_from_path(w_class, w_class2)
...
class Node:
    ...
    def __init__(
        ...
        # Get wClass, wTitle
        if className is None or title is None:
            ...
            className = node["client"]["instanceName"]
            className2 = node["client"]["className"]
            pos_x = node["client"]["tiledRectangle"]["x"]
            pos_y = node["client"]["tiledRectangle"]["y"]
            pos_w = node["client"]["tiledRectangle"]["width"]
            pos_h = node["client"]["tiledRectangle"]["height"]
            ...
            if is_floating:
                raise NodeFloating(f"Floating class: {className}-{className2}")
            if is_sticky:
                raise NodeSticky(f"Sticky class: {className}-{className2}")
            if is_hidden:
                raise NodeHidden(f"Hidden class: {className}-{className2}")
            if className in CLASS_BLACKLIST:
                raise NodeBlacklisted(f"Blacklisted class: {className}-{className2}")
            
        self._class = className
        self._class2 = className2
        self._title = title
        self._pos_x = pos_x
        self._pos_y = pos_y
        ...
        # Set icon
        if icon == "":
            self._icon = Icon.get_path2(self._id, self._class, self._class2)
        else:
            self._icon = icon
        if nIdx is not None:
            self.set_eww()
    ...
    def set_eww(self):
        ...
        eww_cmd_icon += f"window_{self._dIdx}_{self._idx}_icon="
        ...
        eww_cmd_nid += f"window_{self._dIdx}_{self._idx}_nid="
    ...
    def focus(self):
        if self._focussed:
            return
        eww_cmd = "eww update "
        eww_cmd += f"window_{self._dIdx}_{self._idx}_focused="
        eww_cmd += "true"
        print("focus: ", eww_cmd, self.class2)
        subprocess.run(eww_cmd, shell=True)
        self._focussed = True

    def unfocus(self, force=False):
        if not self._focussed and not force:
            return
        eww_cmd = "eww update "
        eww_cmd += f"window_{self._dIdx}_{self._idx}_focused="
        eww_cmd += "false"
        print("unfocus: ", eww_cmd, self.class2)
        subprocess.run(eww_cmd, shell=True)
...
class Desktop:
    ...
    def node_add(self, n: Node, disableRefresh=False) -> Node:
        n_ = Node.new(n)
        # n_.setIdx(len(self._nId2Node) + 1)
    ...
    def focus(self):
        if self._focused:
            return
        eww_cmd = "eww update "
        eww_cmd += f"desktop_{self._idx}_focused="
        eww_cmd += "true"
        subprocess.run(eww_cmd, shell=True)
        self._focused = True
    ...
    def set_eww(self):
        ...
        eww_cmd_dId += f"desktop_{self._idx}_did="
    ...
    def unset_eww(self):
        ...
        # eww_cmd_ts = "eww update "
        # eww_cmd_ts += f"desktop_{self._idx}_ts={time.time():.0f}"
        # subprocess.run(eww_cmd_ts, shell=True)
        ...
        # execute eww_cmd_label after 2 seconds asynchronously
        # eww_cmd_label = "eww update "
        # eww_cmd_label += f"desktop_{self._idx}_label="
        # eww_cmd_label += '""'
        # import threading
        # threading.Timer(1, lambda: subprocess.run(eww_cmd_label, shell=True)).start()
...
class Monitor:
    ...
    def getLiveIndex(self) -> int:
        ...
        return mIdxes.index(self._id) + 1
...
class WM:
    ...
    def desktop_focus(self, mId, dId):
        ...
        d.focus()
        for m_ in self._mId2Monitor.values():
            for d_ in m_.desktops:
                if d_ is not d:
                    d_.unfocus()

    def node_add(self, mId, dId, nId) -> Node:
        m = self._mId2Monitor[mId]
        if m is None:
            return
        d = m[dId]
    ...
    def node_swap(self, src_mId, src_dId, src_nId, dst_mId, dst_dId, dst_nId):
        ...
        print("  * add nodes")
        # src_d.node_add(dst_n, disableRefresh=True)
        dst_n = src_d.node_add_new(dst_nId)
        # dst_d.node_add(src_n, disableRefresh=True)
    ...
    def ev_node_state(self, mId, dId, nId, state, onOff):
        ...
        floating_off = state == "floating" and onOff == "off"
        tiled_on = (state == "tiled" or state == "pseudo_tiled") and onOff == "on"
        tiled_off = (state == "tiled" or state == "pseudo_tiled") and onOff == "off"
        full_on = state == "fullscreen" and onOff == "on"
        full_off = state == "fullscreen" and onOff == "off"
        ...
        # bspc query -T -n <nId>
        # to get last state of the node
        # bspc_cmd = f"bspc query -T -n {nId}"
        # bspc_output = subprocess.check_output(bspc_cmd, shell=True).decode()
        # node = json.loads(bspc_output)
        # last_state = node["client"]["lastState"]
        # cur_state = node["client"]["state"]
    ...
    def _init_from_dump(self):
        ...
        focused_mId = dump["focusedMonitorId"]
        focused_dId = None
        focused_nId = None
        for m in dump["monitors"]:
            if m["id"] == focused_mId:
                focused_m = m
                focused_dId = m["focusedDesktopId"]
                break

        for d in focused_m["desktops"]:
            if d["id"] == focused_dId:
                focused_d = d["id"]
                focused_nId = d["focusedNodeId"]
                break

        if focused_dId is not None:
            eww_cmd = "eww update "
            eww_cmd += f"focused_desktop={focused_dId}"
            subprocess.run(eww_cmd, shell=True)

        if focused_nId is not None:
            eww_cmd = "eww update "
            eww_cmd += f"focused_window={focused_nId}"
            subprocess.run(eww_cmd, shell=True)
